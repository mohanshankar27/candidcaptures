    ...
    >
      ...
      <div className="space-y-8">
        {faqCategories.map((category, categoryIndex) => (
          <div key={categoryIndex} className="bg-white/80 rounded-lg overflow-hidden shadow-sm border border-slate-100">
            ...
            <Accordion type="single" collapsible className="w-full">
              {category.questions.map((faq, index) => (
                <AccordionItem key={index} value={`${categoryIndex}-faq-${index}`}>
                  <AccordionTrigger 
                    className="text-lg font-medium text-gray-800 py-4 hover:text-primary px-4"
                  >
                    {faq.question}
                  </AccordionTrigger>
