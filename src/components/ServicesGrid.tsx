    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-1.5">
      {services.map((service) => (
        <Card 
          key={service.name}
          className={cn(
            "cursor-pointer transition-all overflow-hidden h-16",
        ...
        >
          <CardContent className="p-1 flex items-center gap-1 h-full">
            <div className="text-primary group-hover:scale-110 transition-transform duration-200 flex-shrink-0">
              {service.icon}
            </div>
